...
const DOC_TYPES: Record<string, { label: string; icon: keyof typeof Icons; color: string }> = {
  ...
  MEDICAL_INSURANCE: {
    label: 'تأمين طبي',
    icon: 'badgeCheck',
    color: 'bg-teal-100 text-teal-800 border-teal-200 dark:bg-teal-950/40 dark:text-teal-300'
  },
  OTHER: {
    label: 'مستند آخر',
    icon: 'page',
...
export default function DocumentDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const {
    ...
    isLoading,
    isError
  } = useOfflineQuery<EmployeeDocument>({
      ...
      <PageContainer>
        <div className='flex flex-col items-center justify-center p-12 text-center space-y-4'>
          ...
          <h2 className='text-xl font-bold text-slate-900 dark:text-slate-100'>
            المستند غير موجود أو تم حذفه
          </h2>
          <p className='text-sm text-slate-500'>
            لم يتم العثور على الوثيقة المطلوبة، قد تكون حُذفت أو تم نقلها.
          </p>
  ...
  if (doc.expiry_date) {
    ...
    if (diffDays < 0) {
      ...
      expiryStatus = {
        label: 'ينتهي قريباً',
        color:
          'text-amber-600 bg-amber-50 border-amber-200 dark:bg-amber-950/40 dark:text-amber-400',
  ...
  const isPdf =
    hasValidUrl && (fullFileUrl.startsWith('data:application/pdf') || /\.pdf$/i.test(cleanExtUrl));
  const isVideo =
    hasValidUrl &&
    (fullFileUrl.startsWith('data:video/') || /\.(mp4|mov|webm|avi|mkv|ogg)$/i.test(cleanExtUrl));
  const isAudio =
    hasValidUrl &&
    (fullFileUrl.startsWith('data:audio/') || /\.(mp3|wav|ogg|m4a|aac)$/i.test(cleanExtUrl));
        ...
        <div className='flex flex-wrap items-center justify-between gap-4 pb-1'>
          <Link
            ...
            className={cn(
              ...
              'gap-1.5 text-muted-foreground hover:text-foreground font-semibold'
            )}
          >
            <Icons.arrowRight className='size-4' />
            العودة للمستندات
            ...
            <div className='flex items-start gap-4'>
              ...
              <div className='space-y-1.5'>
                <div className='flex flex-wrap items-center gap-2'>
                  ...
                  <Badge
                    variant='outline'
                    className={cn('font-bold text-xs px-2.5 py-0.5 border', typeConfig.color)}
                  >
                    {typeConfig.label}
                  </Badge>
                  <Badge
                    variant={expiryStatus.badgeVariant}
                  ...
                  <div className='flex items-center gap-2.5'>
                    ...
                    <div>
                      <CardTitle className='text-base font-bold'>
                        معاينة الملف المرفق ومحتواه
                      </CardTitle>
                      <CardDescription
                        className='text-xs truncate max-w-[280px] sm:max-w-md font-mono'
                        dir='ltr'
                      >
                  ...
                  <div className='p-16 text-center space-y-4'>
                    ...
                    <div className='space-y-1'>
                      ...
                      <p className='text-xs text-muted-foreground'>
                        لم يتم إرفاق أو رفع ملف لهذه الوثيقة حتى الآن.
                      </p>
                    </div>
                    <Button
                      onClick={openEditModal}
            ...
            <Card className='shadow-xs border-primary/20 bg-gradient-to-b from-primary/5 to-transparent'>
              <CardHeader className='pb-2 border-b'>
                <CardTitle className='text-sm font-black flex items-center justify-between'>
                  ...
                  <Badge
                    variant='outline'
                    className='text-[10px] text-emerald-600 bg-emerald-50 dark:bg-emerald-950/40 border-emerald-300'
                  >
              ...
              <CardContent className='pt-4 flex flex-col items-center justify-center text-center gap-2.5'>
                  ...
                  <QRCodeImage
                    value={
                      typeof window !== 'undefined'
                        ? `${window.location.origin}/dashboard/documents/${doc.id}`
                ...
                <div className='space-y-0.5'>
                  <p className='text-xs font-bold text-foreground'>
                    امسح بالهاتف لفتح المستند فوراً
                  </p>
                  ...
                  {doc.issue_date && (
                    <div className='flex items-center justify-between py-2'>
                      ...
                      <span className='font-medium text-foreground'>
                        {formatRiyadhDate(doc.issue_date)}
                      </span>
                    </div>
                  )}
                  ...
                  {doc.created_at && (
                    <div className='flex items-center justify-between py-2 text-xs'>
                      ...
                      <span className='font-mono text-muted-foreground'>
                        {formatRiyadhDate(doc.created_at)}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <CardContent className='pt-4 space-y-4'>
                  <div className='flex items-center gap-3'>
                    ...
                    <div className='space-y-0.5'>
                      ...
                      <p className='text-xs text-muted-foreground'>
                        {doc.employee.job_role === 'SUPERVISOR'
                          ? 'مشرف'
                          : doc.employee.job_role === 'MANAGEMENT'
                            ? 'إدارة'
                            : 'مندوب توصيل'}
                      </p>
                    </div>
                  ...
                  <Link
                    ...
                    className={cn(
                      buttonVariants({ variant: 'outline', size: 'sm' }),
                      'w-full gap-2 mt-2 font-semibold'
                    )}
                  >
        ...
        {lightboxOpen && hasValidUrl && (
          ...
          >
            ...
            >
              ...
              <Button variant='destructive' size='sm' onClick={() => setLightboxOpen(false)}>
            ...
            {isImage ? (
              ...
              <div
                className='w-[90vw] h-[85vh] bg-card rounded-2xl overflow-hidden shadow-2xl'
                onClick={(e) => e.stopPropagation()}
              >
                <iframe src={fullFileUrl} className='w-full h-full border-0' title={doc.title} />
              </div>
            )}
            <p className='text-white/80 text-sm mt-4 select-none'>
              {doc.title} — اضغط في أي مكان للإغلاق
            </p>
          </div>
        )}
            ...
            <form onSubmit={handleSaveEdit} className='space-y-4 py-4'>
              <div className='space-y-1.5'>
                ...
                <Select
                  ...
                  onValueChange={(val) => setEditEmployeeId(val || '')}
                >
              ...
              <div className='grid grid-cols-2 gap-3'>
                <div className='space-y-1.5'>
                  ...
                  <Select
                    value={editDocType}
                    onValueChange={(val) => setEditDocType(val || 'CONTRACT')}
                  >
                    <SelectTrigger>
              ...
              <div className='space-y-2'>
                  ...
                  {uploading ? (
                    ...
                    <div className='flex flex-col items-center gap-1.5 py-2'>
                      ...
                      <p className='text-sm font-medium text-muted-foreground'>
                        اسحب ملفاً هنا أو اضغط للاختيار
                      </p>
                      <p className='text-xs text-muted-foreground/60'>
                        صور، PDF، مستندات حتى 50 ميجا
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className='space-y-1'>
                  <Label className='text-xs text-muted-foreground'>
                    أو أدخل رابط الملف المباشر (URL)
                  </Label>
                  <Input
                    value={editFileUrl}
        ...
        <Dialog open={deleteModalOpen} onOpenChange={setDeleteModalOpen}>
          <DialogContent className='max-w-md' dir='rtl'>
            <DialogHeader>
              ...
              <DialogDescription>
                هل أنت متأكد من رغبتك في حذف هذا المستند ({doc.title}) نهائياً؟ لا يمكن التراجع عن هذا
                الإجراء.
              </DialogDescription>
            </DialogHeader>
