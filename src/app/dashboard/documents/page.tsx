...
export default function DocumentsPage() {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      const backendBase =
        process.env.NEXT_PUBLIC_API_URL?.replace('/api/v1', '') || 'https://api.kerd2sy.com';
      ...
      <div className='space-y-6' dir='rtl'>
          ...
          <Card className='border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900'>
            ...
            <CardContent>
              <div className='text-2xl font-bold text-slate-900 dark:text-slate-100'>
                {documents.length - expiredCount - expiringCount}{' '}
                <span className='text-sm font-normal text-slate-500'>وثيقة</span>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              عرض تفاصيل الوثائق وتواريخ الإصدار والانتهاء وحالة التوثيق
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <TableBody>
                  {loading ? (
                    ...
                    filteredDocs.map((doc) => {
                      const docTypeInfo = DOC_TYPES[doc.doc_type] || {
                        label: doc.doc_type,
                        icon: 'fileText'
                      };
