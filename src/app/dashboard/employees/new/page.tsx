    ...
    <PageContainer>
      <div className="space-y-6 max-w-4xl mx-auto pb-10" dir="rtl">
        <PageHeader 
          ...
          actions={
            <Button variant="outline" onClick={() => router.push('/dashboard/employees')} className="gap-2">
            ...
            <CardContent className="pt-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                <div className="space-y-2">
                  <Label htmlFor="employee_number">
                    رقم الجوال (معرف الموظف في النظام)
                  </Label>
                  <Input
                    id="employee_number"
              ...
              {isDriver ? (
                        ...
                        >
                          ...
                          {vehiclesList.map((v) => (
                            <option key={v.id} value={v.plate_number}>
                              {v.type === 'CAR' ? '🚗 سيارة' : '🏍️ دباب'} | لوحة: {v.plate_number} {v.make ? `(${v.make} ${v.model || ''})` : ''} — العداد الحالي: {v.current_km.toLocaleString('en-US')} كم
                            </option>
                          ))}
                        </select>

                        {selectedVehiclePlate && (() => {
                          const v = vehiclesList.find(veh => veh.plate_number === selectedVehiclePlate);
                          if (!v) return null;
                          return (
                            <div className="p-3.5 rounded-xl border border-primary/20 bg-primary/5 flex flex-wrap items-center justify-between gap-3 text-xs">
                              <div className="flex items-center gap-3">
                                <div className="size-9 rounded-lg bg-primary/15 text-primary flex items-center justify-center font-bold">
                                  {v.type === 'CAR' ? <Car className="size-5" /> : <Bike className="size-5" />}
                                </div>
                                <div>
                                  <p className="font-bold text-foreground text-sm">
                                    {v.type === 'CAR' ? 'سيارة' : 'دباب'} لوحة: <span className="font-mono text-primary font-black">{v.plate_number}</span>
                                  </p>
                                  <p className="text-muted-foreground text-[11px]">
                                    {v.make} {v.model} {v.year ? `(${v.year})` : ''} {v.color ? `| اللون: ${v.color}` : ''}
                                  </p>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-3">
                                <div className="px-3 py-1.5 rounded-lg bg-background border font-mono">
                                  <span className="text-muted-foreground text-[10px] block">العداد التراكمي المشترك</span>
                                ...
                                <div className="px-3 py-1.5 rounded-lg bg-background border">
                                  <span className="text-muted-foreground text-[10px] block">حالة تغيير الزيت</span>
                                  <span className="font-medium text-xs flex items-center gap-1">
                                    <Wrench className="size-3.5 text-amber-500" />
                                    {v.oil_interval_km && v.current_km - (v.last_oil_change_km || 0) >= v.oil_interval_km
                                      ? <span className="text-destructive font-bold">يحتاج تغيير زيت</span>
                                      : <span className="text-emerald-600 dark:text-emerald-400 font-bold">سليم</span>}
                                  </span>
                                </div>
                              </div>
                            </div>
                ...
                <div className="p-4 rounded-xl bg-primary/5 border border-primary/20 flex items-center gap-3 text-xs">
                  ...
                  <div>
                    <p className="font-bold text-foreground text-sm">
                      بيانات الوظيفة: {jobRole === 'SUPERVISOR' ? 'مشرف وردية' : jobRole === 'MANAGEMENT' ? 'إدارة' : 'عامل'}
                    </p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      الوظيفة المختارة إدارية/إشرافية ولا تتطلب تخصيص دراجة من الأسطول أو رقم مفتاح أو تطبيق توصيل.
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {isDriver ? (
                  ...
                  <div className="flex flex-col items-center justify-center border-2 border-dashed rounded-2xl p-6 text-center text-muted-foreground bg-muted/20">
                    ...
                    <p className="text-xs font-semibold">رخصة القيادة غير مطلوبة</p>
                    <p className="text-[11px] text-muted-foreground mt-1">خاصة فقط بمناديب وسائقي التوصيل</p>
                  </div>
                )}
              </div>
            </CardContent>
