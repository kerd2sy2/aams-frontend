const fs = require('fs');
let content = fs.readFileSync('src/app/dashboard/work/start/page.tsx', 'utf8');
...
content = content.replace(
  ...
  "onSelectEmployee={(emp) => {\n          if (emp.job_role && emp.job_role !== 'DRIVER') {\n            toast.error('لا يمكن بدء الدوام إلا للمندوبين');\n            return;\n          }\n          selectEmployee(emp);\n          toast.success(`تم إيجاد الموظف: ${emp.name}`);\n        }}"
);

fs.writeFileSync('src/app/dashboard/work/start/page.tsx', content, 'utf8');
